...
contract PreCheckpoint_ProportionalBreakage is Test, BaseTest {
    ...
    function test_PreCheckpoint_Sell_Forfeits_Proportion() public {
        ...
        // ALICE sells 10% before checkpoint
        vm.warp(t0 + 10 days);
        vm.prank(ALICE);
        token.transfer(BOB, 10_000e18);

        // Warp to epoch end and distribute for epoch 1 (non-zero denominator expected)
        vm.warp(t0 + 30 days);
        vm.prank(DISTRIBUTOR);
        rewards.distribute(60_000e6);

        // Rollover to epoch 2 to enable crediting epoch 1 rewards on settlement
        uint64 e2 = t0 + 30 days;
        _configureEpoch(2, e2, e2 + 30 days);

        // Settle and claim to realize epoch 1 rewards, then compare
        vm.prank(ALICE);
        uint256 aliceClaim = rewards.claim(ALICE);
        vm.prank(BOB);
        uint256 bobClaim = rewards.claim(BOB);
        assertLt(aliceClaim, bobClaim, "Seller before checkpoint should forfeit some rewards");
    }
}
